    ...
    <?php $index = 0; foreach ($images as $image): ?>
        <div class="gallery-thumb <?php echo !$carousel ? $grid_classes : ''; ?>">
            <a href="<?php echo $image['image']; ?>" style="<?php echo $image_border; ?>; <?php echo !$carousel && $index >= $thumb_limit ? 'display: none' : ''; ?>" data-thumb="<?php echo $image['thumb']; ?>" class="swipebox" title="<?php echo $image['name']; ?>">
                ...
                <?php if ($carousel): ?>
                <img class="lazyOwl" data-src="<?php echo $image['thumb']; ?>" alt="<?php echo $image['name']; ?>" />
                <?php else: ?>
    ...
    <script>
        $(function(){
            $('#journal-gallery-<?php echo $module; ?> .swipebox').swipebox({
                ...
                beforeOpen: function (){
                    jQuery110("#journal-gallery-<?php echo $module; ?> .box-content").trigger('owl.stop');
                },
                afterClose: function (){
                    jQuery110("#journal-gallery-<?php echo $module; ?> .box-content").trigger('owl.play', <?php echo $autoplay; ?>);
                }
                <?php endif; ?>
            });
        });
